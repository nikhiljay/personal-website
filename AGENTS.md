...
- **Lint:** `bun run lint` is broken — Next.js 16 removed the built-in `next lint` command, so it errors with "Invalid project directory ... /workspace/lint". There is no ESLint config in the repo. Use `bunx tsc --noEmit` for type checking instead.
